from funding_raised1 import FundingRaised1


def test_where_returns_events():
    assert len(FundingRaised1.where({"company_name": "Facebook"})) == 7


def test_where_returns_correct_keys():
    row = FundingRaised1.where({"company_name": "Facebook"})[0]
    keys = [
        "permalink",
        "company_name",
        "number_employees",
...
def test_where_returns_events_by_city():
    assert len(FundingRaised1.where({"city": "Tempe"})) == 3


def test_where_returns_events_by_state():
    assert len(FundingRaised1.where({"state": "CA"})) == 873


def test_where_returns_events_by_company():
    assert len(FundingRaised1.where(
        {"company_name": "Facebook", "round": "a"})) == 1


def test_where_returns_events_by_type():
    assert len(FundingRaised1.where({"round": "a"})) == 582


def test_where_returns_no_events():
    assert len(FundingRaised1.where({"company_name": "NotFacebook"})) == 0


def test_find_by_event_by_company_name():
    row = FundingRaised1.find_by({"company_name": "Facebook"})
    keys = [
        "permalink",
        "company_name",
        "number_employees",
...
def test_find_by_event_by_state():
    row = FundingRaised1.find_by({"state": "CA"})
    keys = [
        "permalink",
        "company_name",
        "number_employees",
